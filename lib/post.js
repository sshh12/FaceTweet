...
friendlyPix.Post = function () {
  /**
                                 ...
                                 * @constructor
                                 */
  function _class() {var _this2 = this;_classCallCheck(this, _class);
    ...
    $(document).ready(function () {
      _this2.postPage = $('#page-post');
      // Pointers to DOM elements.
      _this2.postElement = $(friendlyPix.Post.createPostHtml());
      friendlyPix.MaterialUtils.upgradeTextFields(_this2.postElement);
      _this2.toast = $('.mdl-js-snackbar');
      _this2.theatre = $('.fp-theatre');
    });
  }
  ...
  /**
    ...
    postId) {var _this3 = this;
      ...
      friendlyPix.firebase.getPostData(postId).then(function (snapshot) {
        ...
        if (!post) {
          ...
          _this3.fillPostData(snapshot.key, post.thumb_url || post.url, post.text, post.author,
    ...
    postId, thumbUrl, imageText, author, timestamp, thumbStorageUri, picStorageUri, picUrl) {var _this5 = this;
      ...
      friendlyPix.firebase.registerForThumbChanges(postId, function (thumbUrl) {
    ...
    thumbUrl, picUrl) {var _this7 = this;
      ...
      $('.fp-image', post).click(function () {return _this7.enterTheatreMode(picUrl || thumbUrl);});
    }
    ...
    postId, author, picStorageUri, thumbStorageUri) {var _this9 = this;
      var post = this.postElement;
    ...
    postId) {
      ...
      if (this.auth.currentUser) {
        ...
        $('.fp-not-liked', post).click(function () {return friendlyPix.firebase.updateLike(postId, true);});
      } else {
        $('.fp-liked', post).hide();
        $('.fp-not-liked', post).hide();
        $('.fp-action', post).hide();
    ...
    author, text, postId, commentId) {var isOwner = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
      var element = $('\n        <div class="fp-comment' + (
      isOwner ? ' fp-comment-owned' : '') + '">\n          <a class="fp-author" href="/user/' +
      ...
      $('.fp-delete-comment', element).click(function () {
        if (window.confirm('Delete the comment?')) {
          friendlyPix.firebase.deleteComment(postId, commentId).then(function () {
            element.text('this comment has been deleted');
            element.addClass('fp-comment-deleted');
          });
        }
      });
      $('.fp-edit-comment', element).click(function () {
        var newComment = window.prompt('Edit the comment?', text);
        if (newComment !== null && newComment !== '') {
          friendlyPix.firebase.editComment(postId, commentId, newComment).then(function () {
