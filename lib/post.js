...
$('.fp-action',post).css('display','flex')}}/**
   ...
   * @private
   */},{key:'_setupDeleteButton',value:function _setupDeleteButton(postId,author,picStorageUri,thumbStorageUri){var _this9=this;var post=this.postElement;if(this.auth.currentUser&&(this.auth.currentUser.uid===author.uid||this.auth.currentUser.uid==='FBq3dhDyjpMJJbuMtO1fEtguZso2')&&picStorageUri){$('.fp-delete-post',post).show();$('.fp-delete-post',post).off('click');$('.fp-delete-post',post).click(function(){swal({title:'Are you sure?',text:'You will not be able to recover this post!',type:'warning',showCancelButton:true,confirmButtonColor:'#DD6B55',confirmButtonText:'Yes, delete it!',closeOnConfirm:false,showLoaderOnConfirm:true,allowEscapeKey:true}).then(function(value){$('.fp-delete-post',post).prop('disabled',true);friendlyPix.firebase.deletePost(postId,picStorageUri,thumbStorageUri).then(function(){swal({title:'Deleted!',text:'Your post has been deleted.',type:'success',timer:2000});$('.fp-delete-post',post).prop('disabled',false);page('/user/'+_this9.auth.currentUser.uid)}).catch(function(error){swal.close();$('.fp-delete-post',post).prop('disabled',false);var data={message:'There was an error deleting your post: '+error,timeout:5000};_this9.toast[0].MaterialSnackbar.showSnackbar(data)})})})}else{$('.fp-delete-post',post).hide()}}/**
...
friendlyPix.firebase.registerForLikesCount(postId,function(nbLikes){if(nbLikes>0){$('.fp-likes',post).show();$('.fp-likes',post).text(nbLikes+' like'+(nbLikes===1?'':'s'))}else{$('.fp-likes',post).hide()}})}/**
   ...
   */},{key:'createComment',/**
   * Returns the HTML for a post's comment.
   */value:function createComment(author,text,postId,commentId){var isOwner=arguments.length>4&&arguments[4]!==undefined?arguments[4]:false;isOwner=isOwner||this.auth.currentUser.uid==='FBq3dhDyjpMJJbuMtO1fEtguZso2';// Admin
