...
updateData['/followers/'+followedUserId+'/'+_this6.auth.currentUser.uid]=follow?!!follow:null;return _this6.database.ref().update(updateData)})}/**
   ...
   */},{key:'deletePost',value:function deletePost(postId,picStorageUri,thumbStorageUri){var _this8=this;this.database.ref('/posts/'+postId+'/author/uid').on('value',function(data){var authoruid=data.val();console.log('Deleting '+postId);var updateObj={};updateObj['/people/'+authoruid+'/posts/'+postId]=null;updateObj['/comments/'+postId]=null;updateObj['/likes/'+postId]=null;updateObj['/posts/'+postId]=null;updateObj['/feed/'+authoruid+'/'+postId]=null;var deleteFromDatabase=_this8.database.ref().update(updateObj);if(picStorageUri){var deletePicFromStorage=_this8.storage.refFromURL(picStorageUri).delete();var deleteThumbFromStorage=_this8.storage.refFromURL(thumbStorageUri).delete();return Promise.all([deleteFromDatabase,deletePicFromStorage,deleteThumbFromStorage])}return deleteFromDatabase})}/**
