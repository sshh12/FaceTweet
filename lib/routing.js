 ...
 */friendlyPix.Router=function(){/**
   ...
   * @constructor
   */function _class(){var _this=this;_classCallCheck(this,_class);$(document).ready(function(){friendlyPix.auth.waitForAuth.then(function(){// Dom elements.
...
page()})})}/**
   ...
   */_createClass(_class,[{key:'displayPage',value:function displayPage(attributes,context){var onlyAuthed=attributes.onlyAuthed;var pageId=attributes.pageId;if(onlyAuthed&&!firebase.auth().currentUser){pageId='splash';this.splashLogin.show()}friendlyPix.Router.setLinkAsActive(context.canonicalPath);this.pagesElements.each(function(index,element){if(element.id==='page-'+pageId){$(element).show()}else if(element.id==='page-splash'){$(element).fadeOut(1000)}else{$(element).hide()}});friendlyPix.MaterialUtils.closeDrawer();friendlyPix.Router.scrollToTop()}/**
   * Reloads the current page.
   */},{key:'reloadPage',value:function reloadPage(){var path=window.location.pathname;if(path===''){path='/'}page(path)}/**
   * Scrolls the page to top.
   */}],[{key:'scrollToTop',value:function scrollToTop(){$('html,body').animate({scrollTop:0},0)}/**
   ...
   */},{key:'pipe',value:function pipe(funct,attribute,optContinue){return function(context,next){if(funct){var params=Object.keys(context.params);if(!attribute&&params.length>0){funct(context.params[params[0]],context)}else{funct(attribute,context)}}if(optContinue){next()}}}/**
   * Highlights the correct menu item/link.
   */},{key:'setLinkAsActive',value:function setLinkAsActive(canonicalPath){if(canonicalPath===''){canonicalPath='/'}$('.is-active').removeClass('is-active');$('[href="'+canonicalPath+'"]').addClass('is-active')}}]);return _class}();friendlyPix.router=new friendlyPix.Router;
