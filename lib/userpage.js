...
_this.followCheckbox.change(function(){return _this.onFollowChange()});_this.auth.onAuthStateChanged(function(){return _this.trackFollowStatus()});_this.nbFollowingContainer.click(function(){return _this.displayFollowing()});_this.closeFollowingButton.click(function(){_this.followingContainer.hide();_this.nbFollowingContainer.removeClass('is-active')})})}/**
   ...
   */},{key:'toggleNextPageButton',value:function toggleNextPageButton(nextPage){var _this3=this;if(nextPage){this.nextPageButton.show();this.nextPageButton.unbind('click');this.nextPageButton.prop('disabled',false);this.nextPageButton.click(function(){_this3.nextPageButton.prop('disabled',true);nextPage().then(function(data){_this3.addPosts(data.entries);_this3.toggleNextPageButton(data.nextPage)})})}else{this.nextPageButton.hide()}}/**
   * Displays the given user information in the UI.
   */},{key:'loadUser',value:function loadUser(userId){var _this4=this;this.userId=userId;// Reset the UI.
...
if(this.auth.currentUser&&userId===this.auth.currentUser.uid){this.followContainer.hide();friendlyPix.messaging.enableNotificationsContainer.show();friendlyPix.messaging.enableNotificationsCheckbox.prop('disabled',true);friendlyPix.MaterialUtils.refreshSwitchState(friendlyPix.messaging.enableNotificationsContainer);friendlyPix.messaging.trackNotificationsEnabledStatus()}else{friendlyPix.messaging.enableNotificationsContainer.hide();this.followContainer.show();this.followCheckbox.prop('disabled',true);friendlyPix.MaterialUtils.refreshSwitchState(this.followContainer);// Start live tracking the state of the "Follow" Checkbox.
this.trackFollowStatus()}// Load user's profile.
friendlyPix.firebase.loadUserProfile(userId).then(function(snapshot){var userInfo=snapshot.val();if(userInfo){_this4.userAvatar.css('background-image','url("'+(userInfo.profile_picture||'/images/silhouette.jpg')+'")');_this4.userUsername.text(userInfo.full_name||'Anonymous');_this4.userInfoContainer.show()}else{var data={message:'This user does not exists.',timeout:5000};_this4.toast[0].MaterialSnackbar.showSnackbar(data);page('/feed')}});// Lod user's number of followers.
friendlyPix.firebase.registerForFollowersCount(userId,function(nbFollowers){return _this4.nbFollowers.text(nbFollowers)});// Lod user's number of followed users.
friendlyPix.firebase.registerForFollowingCount(userId,function(nbFollowed){return _this4.nbFollowing.text(nbFollowed)});// Lod user's number of posts.
friendlyPix.firebase.registerForPostsCount(userId,function(nbPosts){return _this4.nbPostsContainer.text(nbPosts)});// Display user's posts.
friendlyPix.firebase.getUserFeedPosts(userId).then(function(data){var postIds=Object.keys(data.entries);if(postIds.length===0){_this4.noPosts.show()}friendlyPix.firebase.subscribeToUserFeed(userId,function(postId,postValue){_this4.userInfoPageImageContainer.prepend(friendlyPix.UserPage.createImageCard(postId,postValue.thumb_url||postValue.url,postValue.text));_this4.noPosts.hide()},postIds[postIds.length-1]);// Adds fetched posts and next page button if necessary.
_this4.addPosts(data.entries);_this4.toggleNextPageButton(data.nextPage)});// Listen for posts deletions.
friendlyPix.firebase.registerForPostsDeletion(function(postId){return $('.fp-post-'+postId,_this4.userPage).remove()})}/**
   * Displays the list of followed people.
   */},{key:'displayFollowing',value:function displayFollowing(){var _this5=this;friendlyPix.firebase.getFollowingProfiles(this.userId).then(function(profiles){// Clear previous following list.
$('.fp-usernamelink',_this5.followingContainer).remove();// Display all following profile cards.
Object.keys(profiles).forEach(function(uid){return _this5.followingContainer.prepend(friendlyPix.UserPage.createProfileCardHtml(uid,profiles[uid].profile_picture,profiles[uid].full_name))});if(Object.keys(profiles).length>0){_this5.followingContainer.show();// Mark submenu as active.
...
friendlyPix.firebase.registerForLikesCount(postId,function(nbLikes){return $('.likes',element).text(nbLikes)});friendlyPix.firebase.registerForCommentsCount(postId,function(nbComments){return $('.comments',element).text(nbComments)});return element}/**
   * Returns an image Card element for the image with the given URL.
   */},{key:'createProfileCardHtml',value:function createProfileCardHtml(uid){var profilePic=arguments.length>1&&arguments[1]!==undefined?arguments[1]:'/images/silhouette.jpg';var fullName=arguments.length>2&&arguments[2]!==undefined?arguments[2]:'Anonymous';return'\n        <a class="fp-usernamelink mdl-button mdl-js-button" href="/user/'+uid+'">\n            <div class="fp-avatar" style="background-image: url(\''+profilePic+'\')"></div>\n            <div class="fp-username mdl-color-text--black">'+fullName+'</div>\n        </a>'}}]);return _class}();friendlyPix.userPage=new friendlyPix.UserPage;
